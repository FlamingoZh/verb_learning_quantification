...
### Computing Variance and Distinctness ###
...
### Sampling and Aggregating Exemplars ###
...
# just sample, not aggregate, visual embeddings
def sample_embeddings_visual(input_struct,n_sample_per_category):
	words=input_struct['words']
	embed_dict=dict()
	for word in words:
		n_sample=min(len(input_struct['embeds'][word]['visual']),n_sample_per_category)
		visual_temp=np.array(random.sample(input_struct['embeds'][word]['visual'],n_sample))
		language_temp=np.mean(np.array(input_struct['embeds'][word]['language']),axis=0)
		embed_dict[word]=dict(visual=visual_temp,language=language_temp)
	return dict(embeds=embed_dict,words=words)
...
# just sample, not aggregate, language embeddings
def sample_embeddings_language(input_struct,n_sample_per_category):
	words=input_struct['words']
	embed_dict=dict()
	for word in words:
		visual_temp=np.mean(np.array(input_struct['embeds'][word]['visual']),axis=0)
		n_sample = min(len(input_struct['embeds'][word]['language']), n_sample_per_category)
		language_temp=np.array(random.sample(input_struct['embeds'][word]['language'],n_sample))
		embed_dict[word]=dict(visual=visual_temp,language=language_temp)
	return dict(embeds=embed_dict,words=words)
...
# just sample, not aggregate, visual embeddings and langauge embeddings
def sample_embeddings_visual_and_language(input_struct,n_sample_per_visual,n_sample_per_language):
	words=input_struct['words']
	embed_dict=dict()
	for word in words:
		n_sample_v = min(len(input_struct['embeds'][word]['visual']), n_sample_per_visual)
...
### Code for Calling Language Models ###

def sample_sentence_from_corpus(concepts,n_sample,corpus_name='wiki_en',window_size=50,pos=None,do_inflect=False):
	# import linecache
	...
	print("number of paragraphs in the corpus:",len(corpus_content))
	...
	for i, target_word in enumerate(concepts):
		...
		if do_inflect:
			...
			# get all possible word forms of the target word
			if pos=="noun":
				word_set.add(getInflection(target_word, tag='NN'))
				word_set.add(getInflection(target_word, tag='NNS'))
			elif pos=="verb":
				word_set.add(getInflection(target_word, tag='VBD'))
				word_set.add(getInflection(target_word, tag='VBG'))
				word_set.add(getInflection(target_word, tag='VBN'))
				word_set.add(getInflection(target_word, tag='VBP'))
				word_set.add(getInflection(target_word, tag='VBZ'))
			else:
		...
		if i_sample>=n_sample:
			...
			packed_samples.append((sampled_contexts, corresponding_words))
	print("Corpus Sampling Completed.")
	return packed_samples

def call_pretrained_BERT(concepts,packed_samples,cuda=False):
	...
	for concept, samples_of_the_concept in zip(concepts,packed_samples):
		...
		print("Deriving word embeddings of", corresponding_words[0], '('+str(i_concept)+'/'+str(len(concepts))+')'+"...")
		...
		for text, target_word in zip(sampled_contexts,corresponding_words):
			...
			last_hidden_states = outputs['last_hidden_state'] # size (batch_size, token_num, embedding_size)
			last_hidden_states= torch.squeeze(last_hidden_states) # remove first dimension
			
			word_index=text.split().index(target_word)
			word_start,word_end=encodings.word_to_tokens(word_index)
			word_embedding=last_hidden_states[word_start:word_end]
			if cuda:
...
def call_pretrained_GPTJ(concepts,packed_samples,cuda=False):
	print("Forward Propagation in GPT-J.")

	import torch
	# from transformers import BertModel, BertTokenizer
	from transformers import GPT2TokenizerFast, GPTJModel
	...
	# load pretrained gpt-j-6B; if you haven't downloaded the model yet, it will be downloaded automatically
...
def call_pretrained_BERT_depreciated(concepts,packed_samples,cuda=False,mask=False):
	print("Forward Propagation in BERT.")
